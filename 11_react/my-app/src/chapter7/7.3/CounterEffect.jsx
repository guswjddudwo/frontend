  ...
  return (
    <div>
      ...
      <button onClick={() => {
        ...
        // use Effect()에 넣는것이랑 버튼 클릭안에 넣는것이랑 실행 시점이 다르기에 동작이 다름
        // 1. 처음 마운트 시에 실행안됨(타이틀이 안바뀌어 있음)
...
// 3) 마운트 됬을 때 그 페이지를 이용할 수 있는 사용자인지 권한 체크
// 4) 처음 렌더링 시 초기화 해줘야 할 작업들
// 5) 실행하는 로직이 있는 곳과 실행 결과를 보여주는 컴포넌트가 다른 곳이면 로직이 분리되서 가독성이 떨어지고 디버깅이 어려움
// 6) 데이터 패치관련 작업을 많이 넣음
// 만약 버튼을 눌렀을 때 데이터를 가져오는로직을 넣게되면 새로고침 시 초기화됨 -> 다시 버튼을 눌러야 됨

// 정리!!
// onClick과 useEffect는 다른 용도
// onClick 속성으로 넣어준 콜백 함수는 "클릭시 호출되는" 함수이고,
// useEffect 내부에 있는 콜백 함수는 "렌더린시 (의존성 배열에 따라) 호출되는" 함수임
// 어떻게 사용하는지에 다라 겹치는 결과도 있겠지만, 완전히 다른 의도를 가짐!