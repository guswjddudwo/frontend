...
function ApiRequest(props) {
    ...
    const handleRequestById = (id) => {
        //  jsonplaceholderㅇㅔ서 제공하는 테스트용 API 호출
        axios.get(`https://jsonplaceholder.typicode.com/photos/${id}`)
        .then((response) => {
            console.log(response);
            setData(response.data); // axios 라이브러리가 JSON -> object.array 번환 작업을 자동으로 해줌
        })
        .catch((error) => { // 요청 실패할 경우 에러 핸들링
            console.error(error);
        })
    };
    ...
    //         console.log(response);
    //         setData(response.data); // axios 라이브러리가 JSON -> object.array 번환 작업을자동으로 해줌
